...
require_once("../imageControl/extensionGroups.php");
...
?>
    ...
    <body class="ocp_body" onresize="resizeImage()">
        <div align="center" class="ocp_opcije_tekst1" style="padding: 5px 0px 0px 0px">
            <?php
            // ucitavamo sliku
            if (ereg($fileExt, $extensionGroups[1])) {
                ?>
            ...
            <a href="<?php echo $fileName?>" target="_blank"><?php echo $fileNameShort?></a>
                <?php } else if (ereg($fileExt, $extensionGroups[2])) { ?>
            <a href="#" onclick="var x = window.open('/ocp/controls/imageControl/swf_popup.php?url=<?php echo $fileName?>', '', 'width=500, height=400, resizable, scrollbars' ); return false;"><img src="/ocp/img/kontrole/file_kontrola/<?php echo $fileExt?>.gif" onLoad="getDims();" border="0" align="center"></a><br>
            <a href="#" onclick="var x = window.open('/ocp/controls/imageControl/swf_popup.php?url=<?php echo $fileName?>', '', 'width=500, height=400, resizable, scrollbars' ); return false;"><?php echo $fileNameShort?></a>
                <?php } else if (ereg($fileExt, $extensionGroups[3])) { ?>
            ...
            <form name="forma" id="forma" action="/ocp/html/blank.html" onsubmit="return submitFiles();">
                ...
                <p /><input type="button" id="crop" name="crop" value="<?php echo ocpLabels("Thumbs")?>" class="ocp_forma" style="visibility:hidden" align="center" onclick="openThumbnails('<?php echo $type?>');"/>
                <?php
                if (getSVar('ocpAllowedDeleteFiles') == "1") {
                    ?><br/><br/>
...
function openThumbnails(type){
    var _rightFrame = parent.top.document.getElementById("rightFrameset");
    var _uploadFrame =  (_rightFrame.contentDocument || _rightFrame.contentWindow.document).getElementById('uploadFrame');
    var _uploadDocument = (_uploadFrame.contentDocument || _uploadFrame.contentWindow.document)
    var imageSrc = img.src;
    (_rightFrame.contentDocument || _rightFrame.contentWindow.document).getElementById("resizableFrameset").setAttribute("rows", "*,99%");
    _uploadFrame.src = 'thumbs.php?objType=' + type + '&random=' + Math.random() + '&file=<?php echo $fileName; ?>';
}
        </script>
    </body>
</html>