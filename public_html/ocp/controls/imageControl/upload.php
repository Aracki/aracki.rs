    ...
    if (isset($_FILES[$param_name]) && !is_null($_FILES[$param_name])) {
        ...
        if ($error) {
            ...
            if (shouldCreateThumbnail()) {
                $thumbnailSizes = thumbnailer::getThumbnailSizes($uploadfile);
                $destinationPath = $_REQUEST["destination"];
                $destinationDir = $webRoot . preg_replace("#upload/#", "upload/thumbs/", $destinationPath);
                if (!file_exists($destinationDir)) {
                    mkdir($destinationDir, 0775, true);
                }
                foreach($thumbnailSizes as $size) {
                    thumbnailer::create_thumbnail($uploadfile, $destinationDir, $size);
                }
            }
        }
    }
...
?>
    ...
    function overwrite(file){
        ...
        if ($message != "") { ?>
        <script type="text/javascript"> parent.listFrame.location.reload(true);</script><?php
