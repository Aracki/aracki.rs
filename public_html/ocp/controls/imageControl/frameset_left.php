...
$filePath = preg_replace("#//#", "/", $basicFolder.$fileName);
require_once($_SERVER["DOCUMENT_ROOT"] . "/ocp/include/fb.php");
...
if (utils_valid($filePath)) {
    $frameSrc = "preview.php?".utils_randomQS()."&listType=list&root=" . $root . "&field=" . $field . "&sirina=" . $sirina . "&visina=" . $visina . "&max=" . $max . "&putanja=" . $basicFolder . "&fileName=" . $filePath . "&objType=" . $type;
} else {
...
?><html>
    <head>
        ...
        <script>
            ...
            framesetStr += '<frame src="left.php?<?php echo utils_randomQS()?>&root=<?php echo $root?>&field=<?php echo $field?>&sirina=<?php echo $sirina?>&visina=<?php echo $visina?>&max=<?php echo $max?>&basicFolder=<?php echo $basicFolder?>&objType=<?php echo $type?>" id="treeFrame" name="treeFrame" FRAMEBORDER="'+frameBorder+'" BORDER="5" scrolling="NO">';
