        ...
        <table class="ocp_uni_table">
            <tr>
                ...
                <td class="ocp_dugmici_td_desni_2">
                    <a>
                        <xsl:attribute name="href">javascript:x=window.open('/ocp/controls/imageControl/frameset.php?objType=<xsl:value-of select="$type"/>&amp;random=<xsl:value-of select="./@random"/>&amp;width=<xsl:value-of select="$width"/>&amp;height=<xsl:value-of select="$height"/>&amp;field=formObject.<xsl:value-of select="$name"/>&amp;basicFolder='+document.formObject.<xsl:value-of select="$name"/>.value+'&amp;root=<xsl:value-of select="$rootFolder"/>&amp;max=<xsl:value-of select="$max"/>','imgKontrola','top=100, left=50, width=760, height=560, scrollbars=yes, resizable=yes, status=yes');x.focus();</xsl:attribute>
